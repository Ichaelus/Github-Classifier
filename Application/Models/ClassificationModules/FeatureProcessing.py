...
def getMetadataVector(sample):
    # Get metadata
    vec = []
    vec.append(np.nanmin(1., float(sample['hasDownloads'])))
    vec.append(np.nanmin(1., float(sample['watches']) / max_watches))
    vec.append(np.nanmin(1., float(sample['folder_count']) / max_folder_count))
    vec.append(np.nanmin(1., float(sample['treeDepth']) / max_treeDepth))
    vec.append(np.nanmin(1., float(sample['stars']) / max_stars))
    vec.append(np.nanmin(1., float(sample['branch_count']) / max_branch_count))
    vec.append(np.nanmin(1., float(sample['forks']) / max_forks))
    vec.append(np.nanmin(1., float(sample['commit_interval_avg']) / max_commit_interval_avg))
    vec.append(np.nanmin(1., float(sample['contributors_count']) / max_contributors_count))
    vec.append(np.nanmin(1., float(sample['open_issues_count']) / max_open_issues_count))
    vec.append(np.nanmin(1., float(sample['avg_commit_length']) / max_avg_commit_length))
    vec.append(np.nanmin(1., float(sample['hasWiki'])))
    vec.append(np.nanmin(1., float(sample['file_count']) / max_file_count))
    vec.append(np.nanmin(1., float(sample['commit_interval_max']) / max_commit_interval_max))
    vec.append(np.nanmin(1., float(sample['isFork'])))
    vec.append(np.nanmin(1., float(sample['commit_count']) / max_commit_count))
    vec.append(np.nanmin(1., float(getReadmeLength(sample) / max_readme_length)))
